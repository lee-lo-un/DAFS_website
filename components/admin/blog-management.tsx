...
export default function BlogManagement() {
  ...
  function extractStoragePathFromUrl(
    ...
    try {
      ...
      console.log("파싱할 이미지 URL:", url);
      
      // 형식 1: 완전한 URL 형식 (https://dfhimjheydigpmgvspov.supabase.co/storage/v1/object/public/images/blog/...)
      const fullUrlPattern = /https:\/\/([^\/]+)\.supabase\.co\/storage\/v1\/object\/public\/([^\/]+)\/(.+)$/;
      const fullUrlMatch = url.match(fullUrlPattern);
      if (fullUrlMatch) {
        console.log("완전한 URL 패턴 매칭:", { bucket: fullUrlMatch[2], filePath: fullUrlMatch[3] });
        return {
          bucket: fullUrlMatch[2], // 'images'
          filePath: decodeURIComponent(fullUrlMatch[3])  // URL 디코딩 추가
        };
      }
      
      // 형식 2: 상대 경로 형식 (/storage/v1/object/public/bucket/path)
      const relativePattern = /\/storage\/v1\/object\/public\/([^/]+)\/(.+)$/;
      const relativeMatch = url.match(relativePattern);
      if (relativeMatch) {
        console.log("상대 경로 패턴 매칭:", { bucket: relativeMatch[1], filePath: relativeMatch[2] });
        return {
          bucket: relativeMatch[1],
          filePath: decodeURIComponent(relativeMatch[2]) // URL 디코딩 추가
        };
      }
      
      // 형식 3: 단순 경로 형식 (bucket/path)
      const simplePattern = /^([^/]+)\/(.+)$/;
      const simpleMatch = url.match(simplePattern);
      if (simpleMatch) {
        console.log("단순 경로 패턴 매칭:", { bucket: simpleMatch[1], filePath: simpleMatch[2] });
        return {
          bucket: simpleMatch[1],
          filePath: decodeURIComponent(simpleMatch[2]) // URL 디코딩 추가
        };
      }
      
      // 매칭되는 패턴이 없을 경우
      console.warn("지원되지 않는 Supabase 이미지 URL 형식:", url);
      return null;
    } catch (error) {
      console.error("이미지 URL 파싱 오류:", error, url);
      return null;
    }
  }
    ...
    try {
      ...
      for (const imageUrl of allImages) {
        try {
          ...
          debugLog += `  파일 존재 여부 확인: ${fileExistsInBucket ? '존재함' : '존재하지 않음'}\n`;
          
          if (!fileExistsInBucket) {
            debugLog += `  파일이 존재하지 않아 삭제 건너뜀\n`;
            deletedImgs.push(imageUrl); // 이미 존재하지 않으므로 삭제된 것으로 처리
            continue;
          }
          ...
          // 삭제 실패 시 추가 정보 로깅
          if (error) {
            debugLog += `  삭제 오류 상세: ${JSON.stringify(error)}\n`;
            debugLog += `  삭제 권한 문제일 수 있음. Supabase Storage RLS 정책을 확인하세요.\n`;
            
            // 권한 오류인 경우 관리자 API로 시도
            if (error.message.includes('permission denied') || error.message.includes('not authorized')) {
              debugLog += `  관리자 권한으로 삭제 시도 중...\n`;
              try {
                // 서버리스 함수를 통한 삭제 시도 (관리자 권한)
                const { data: adminData, error: adminError } = await supabase.functions.invoke('delete-storage-file', {
                  body: { bucket, filePath }
                });
                
                if (adminError) {
                  debugLog += `  관리자 삭제 오류: ${JSON.stringify(adminError)}\n`;
                } else {
                  debugLog += `  관리자 권한으로 삭제 성공\n`;
                  deletedImgs.push(imageUrl);
                  continue;
                }
              } catch (adminCallError) {
                debugLog += `  관리자 API 호출 오류: ${adminCallError}\n`;
              }
            }
          }
  ...
  async function verifyStorage() {
    try {
      ...
      toast({
        title: "스토리지 확인 완료",
        description:
          "스토리지 파일 목록 확인이 완료되었습니다. 디버그 정보를 확인하세요.",
      });
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      ...
      <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              정말로 "{selectedPost?.title}" 포스트를 삭제하시겠습니까? 이
              작업은 되돌릴 수 없으며, 포스트에 포함된 모든 이미지도 함께
              삭제됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
